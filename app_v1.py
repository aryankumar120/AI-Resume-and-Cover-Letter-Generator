...
if not GROQ_API_KEY:
    raise ValueError("No Groq API key found. Please set it in your environment variables.")

# Choose a supported Groq model, e.g., "llama2-70b-4096" or "mixtral-8x7b-32768"
groq_llm = ChatGroq(
...
Generate your professional cover letter or resume with AI assistance - and land a great job!
""")
...
# Function to generate cover letter using Groq LLM
def generate_cover_letter(prompt, temperature=0.99):
    try:
        # langchain_groq's ChatGroq uses .invoke(prompt) for generation
...
# Function to generate resume using Groq LLM
